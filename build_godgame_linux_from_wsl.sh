...
if [ "$FORCE_LINUX_UNITY" = "1" ]; then
  ...
  if command -v wslpath >/dev/null 2>&1; then
    ...
    if [ -x "$UNITY_WSL" ]; then
      ...
      PROJECT_DIR="${TRI_WSL}/godgame"
      PROJECT_DIR_WIN="${TRI_WIN}\\godgame"
      LOG_PATH="${TRI_WSL}/godgame_headless_build.log"
      LOG_PATH_WIN="${TRI_WIN}\\godgame_headless_build.log"
      BUILD_SRC="${PROJECT_DIR}/Builds/Godgame_headless/Linux"
    fi
  fi
...
if [ "$UNITY_MODE" = "linux" ]; then
  PROJECT_DIR="${TRI_ROOT}/godgame"
  LOG_PATH="${TRI_ROOT}/godgame_headless_build.log"
  BUILD_SRC="${PROJECT_DIR}/Builds/Godgame_headless/Linux"
fi
...
SPACE4X_DIR="${TRI_ROOT}/space4x"

BUILD_BIN="${BUILD_SRC}/Godgame_Headless.x86_64"
PUBLISH_ROOT="${PUBLISH_ROOT:-${TRI_ROOT}/Tools/builds/godgame}"
LATEST_DIR="${PUBLISH_ROOT}/Linux_latest"
STAMP_FILE="${LATEST_DIR}/build_stamp.txt"
GODGAME_HASH="$(resolve_git_hash "${PROJECT_DIR}")"
...
echo "BUILD_STAMP:project=Godgame sha=${GODGAME_HASH} utc=${BUILD_UTC} unity=${UNITY_VERSION} platform=linux headless=1 bank_rev=${BANK_REV}"
echo "Unity: ${UNITY_PATH} (${UNITY_MODE})"

echo "pwd:"
pwd
echo "space4x HEAD:"
log_git_head "${SPACE4X_DIR}"
echo "puredots HEAD:"
log_git_head "${PUREDOTS_DIR}"
echo "godgame HEAD:"
log_git_head "${PROJECT_DIR}"

if [ "$GODGAME_HASH" != "unknown" ] && [ "$PUREDOTS_HASH" != "unknown" ] && [ -f "$STAMP_FILE" ] && [ -f "$BUILD_BIN" ]; then
  STAMP_GODGAME="$(sed -n "s/^godgame_commit=//p" "$STAMP_FILE" | head -n 1)"
  STAMP_PUREDOTS="$(sed -n "s/^puredots_commit=//p" "$STAMP_FILE" | head -n 1)"
  if [ "$STAMP_GODGAME" = "$GODGAME_HASH" ] && [ "$STAMP_PUREDOTS" = "$PUREDOTS_HASH" ]; then
    echo "Build up-to-date for godgame=${GODGAME_HASH} puredots=${PUREDOTS_HASH}; skipping rebuild."
    exit 0
  fi
fi
...
cp -a "$BUILD_SRC" "$DST"
chmod +x "$DST/Godgame_Headless.x86_64" || true
rm -rf "$LATEST_DIR"
cp -a "$DST" "$LATEST_DIR"
LATEST_BIN="${LATEST_DIR}/Godgame_Headless.x86_64"
BIN_MTIME="$(stat -c %y "$LATEST_BIN" 2>/dev/null || true)"
cat > "$STAMP_FILE" <<EOF
godgame_commit=${GODGAME_HASH}
