...
## Tiered nightlies (baseline packs)
Config:
- `Tools/Polish/pipeline_tiers.json`

Run tiered packs:
```powershell
powershell -File Tools/Polish/run_nightly.ps1 -Title space4x -Tier tier0 -UnityExe "C:\Program Files\Unity\Hub\Editor\6000.3.1f1\Editor\Unity.exe"
powershell -File Tools/Polish/run_nightly.ps1 -Title space4x -Tier tier1 -UnityExe "C:\Program Files\Unity\Hub\Editor\6000.3.1f1\Editor\Unity.exe"
```

## Canonical Queue Roots (Windows <-> WSL)
Per-title queue roots (current):
- Space4X: `C:\polish\anviloop\space4x\queue` -> `/mnt/c/polish/anviloop/space4x/queue`
- Godgame: `C:\polish\anviloop\godgame\queue` -> `/mnt/c/polish/anviloop/godgame/queue`
...
## WSL runner
Runner docs:
- `Tools/Polish/WSL/README.md`

Example:
```bash
./Tools/Polish/WSL/wsl_runner.sh --queue /mnt/c/polish/anviloop/space4x/queue --daemon --print-summary --reports-dir /mnt/c/polish/anviloop/space4x/queue/reports
```
...
Dry-run (default):
```powershell
pwsh -File Tools/Polish/cleanup_queue.ps1 -QueueRoot "C:\polish\anviloop\space4x\queue" -RetentionDays 21 -KeepLastPerScenario 5
```

Apply deletions:
```powershell
pwsh -File Tools/Polish/cleanup_queue.ps1 -QueueRoot "C:\polish\anviloop\space4x\queue" -RetentionDays 21 -KeepLastPerScenario 5 -Apply
```
...
## Path Mapping Rules
- Windows path `C:\polish\anviloop\<title>\queue\...` is read in WSL as `/mnt/c/polish/anviloop/<title>/queue/...`.
- `artifact_uri` in job JSON should use the WSL-visible path:
  - `/mnt/c/polish/anviloop/<title>/queue/artifacts/artifact_<build_id>.zip`
...
```json
{
  ...
  "param_overrides": {},
  "feature_flags": {},
  "artifact_uri": "/mnt/c/polish/anviloop/space4x/queue/artifacts/artifact_20260108_003012_abcd1234.zip",
