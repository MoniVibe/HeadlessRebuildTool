# Addendum: <Scenario Name> - Nuance Notes
Date: <YYYY-MM-DD>
Owner: <name>
Status: draft | active | archived

## Purpose
<Why this addendum exists and what depth it captures.>

## Nuance Map
- <Profile doctrine nuance>
- <Experience tier nuance>
- <Tooling/drone nuance>
- <Edge-case behavior>

## Experience and Doctrine Expression
- Experience deltas: <rookie vs elite differences>
- Profile x experience: <warlike elites aggressive; spiritual elites psionics/mana; materialists deploy drones>
- Doctrine/tooling: <materialist drones; corrupt suicide drones; pure repair/support drones>

## Profile Interplay Notes
- Interplay focus: <negotiation | cooperation | rivalry | conflict | environment-driven>
- Expected expressions: <how profiles show up in behavior>

## Nuance Budget / Perf Notes (Optional)
- Budget target: <max expensive decisions per entity per minute>
- Expensive features: <list>
- Fallbacks when budget exceeded: <list>
- Stress-test references: <scenario ids, run ids>

## Telemetry Wish-list
- <metric key>
- <metric key>

## Open Questions
- <unknowns or TODOs>
